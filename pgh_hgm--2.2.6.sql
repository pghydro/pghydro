...
-------------------------------------------------------------------------------------------
--PGH_HGM: PgHydro Hydrological Geomorphology Calculations Extension - v.2.2.6 (2025.06.24)
-------------------------------------------------------------------------------------------
...
-- FUNCTION: pgh_hgm.pghfn_dra_elevations(integer)

-- DROP FUNCTION IF EXISTS pgh_hgm.pghfn_dra_elevations(integer);

CREATE OR REPLACE FUNCTION pgh_hgm.pghfn_dra_elevations(
	dra_pk_ integer)
    RETURNS TABLE(_dra_pk integer, mean_ double precision, max_ double precision, min_ double precision) 
    LANGUAGE 'plpgsql'
    COST 100
...
WITH clipped_tiles AS (
  SELECT ST_Clip(dem.rast, dra.geom) AS rast, dem.rid
  FROM pgh_raster.pghrt_elevation dem
  JOIN (SELECT ST_TRANSFORM(dra_gm, srid_dem) geom  FROM pghydro.pghft_drainage_area WHERE dra_pk = dra_pk_) dra
    ON ST_Intersects(dra.geom, ST_ConvexHull(dem.rast))
)
SELECT dra_pk_, (ST_SummaryStatsAgg(rast, 1, true)).mean, (ST_SummaryStatsAgg(rast, 1, true)).max, (ST_SummaryStatsAgg(rast, 1, true)).min
  FROM clipped_tiles;
...
ALTER FUNCTION pgh_hgm.pghfn_dra_elevations(integer)
    OWNER TO postgres;
...
-- FUNCTION: pgh_hgm.pghfn_dra_elevations_stats(integer)

-- DROP FUNCTION IF EXISTS pgh_hgm.pghfn_dra_elevations_stats(integer);

CREATE OR REPLACE FUNCTION pgh_hgm.pghfn_dra_elevations_stats(
	dra_pk_ integer)
    RETURNS TABLE(dra_elevation_avg double precision, dra_elevation_max double precision, dra_elevation_min double precision, dra_elevationdrop_maxmin double precision) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE PARALLEL UNSAFE
    ROWS 1000
...
BEGIN
    ...
    QUERY
    SELECT
        mean_ as dra_elevation_avg,    
        max_ as dra_elevation_max,
        min_ as dra_elevation_min,
        max_ - min_ AS dra_elevationdrop_maxmin
    FROM
...
BEGIN
	...
	-- CREATE SPATIAL INDEXES ON UPSTREAM GEOMETRIES
	DROP INDEX IF EXISTS pgh_hgm.hig_upa_gm_idx;
	CREATE INDEX IF NOT EXISTS hig_upa_gm_idx
	ON pgh_hgm.pghft_hydro_intel USING gist (hig_upa_gm);
	
	DROP INDEX IF EXISTS pgh_hgm.hig_upn_gm_idx;
	CREATE INDEX IF NOT EXISTS hig_upn_gm_idx
	ON pgh_hgm.pghft_hydro_intel USING gist (hig_upn_gm);

	-- FUNCTIONAL INDEXES ON USPTREAM GEOMETRIES
	DROP INDEX IF EXISTS pgh_hgm.hig_upa_gm_fidx_3857;
	CREATE INDEX IF NOT EXISTS hig_upa_gm_fidx_3857
	ON pgh_hgm.pghft_hydro_intel
	USING gist (ST_TRANSFORM(hig_upa_gm,3857));

	DROP INDEX IF EXISTS pgh_hgm.hig_upn_gm_fidx_3857;
	CREATE INDEX IF NOT EXISTS hig_upn_gm_fidx_3857
	ON pgh_hgm.pghft_hydro_intel
	USING gist (ST_TRANSFORM(hig_upn_gm,3857));
	---
